...
parser.add_argument('--trace-ram-usage',
                    action='store_true',
                    help='Trace RAM usage for debugging purposes; only available with python > 3.4',
                    dest='trace_ram',
                    default=False)
...
#######################################################################################################
flux, hdr = read.read_input_images(line_names, lines_available, param_model_values, "fluxes")


#######################################################################################################
# read line flux error images
#######################################################################################################
flux_error, hdrer = read.read_input_images(line_names, lines_available, param_model_values, "errors")
...
if param_requered[index] == "yes":
    flux_angles, flux_angles_error, flux_angles_norm, flux_angles_norm_error, ratio_angles, ratio_angles_error, angles = rotlfs.rotate_line_fluxes(
        flux, flux_error, line_names, line_ext_error, lines_available, param_estimated, param_requered, param_mod_name, param_model_values, par_plotname, par_plotymin, par_plotymax)
...
#######################################################################################################
    Te_PA, Ne_PA = TeNeangles.TeNe(flux_angles, flux_angles_error, angles, line_names, lines_available, param_estimated,
                                   param_requered, param_mod_name, param_model_values, par_plotname, par_plotymin, par_plotymax)
...
if param_requered[index] == "yes":
    flux_radial, flux_radial_error, distance_CS = ras.radial_analysis(
        flux, flux_error, line_names, line_ext_error, lines_available, lines_radial, param_mod_name, param_model_values, par_plotname, par_plotymin, par_plotymax)

#######################################################################################################
# use the derived fluxes and ratios as input to Pyneb and calculate paramteres such as
# Te, Ne, ionic and chemical abudnances,  and ICFs
#######################################################################################################
    Te_rad, Ne_rad = TeNers.TeNe(flux_radial, flux_radial_error, distance_CS, line_names, lines_available,
                                 param_estimated, param_requered, param_mod_name, param_model_values, par_plotname, par_plotymin, par_plotymax)
...
if param_requered[index] == "yes":
    flux_spec_slit, flux_spec_slit_error, flux_spec_slit_norm, flux_spec_slit_norm_error, ratio_spec_slit, ratio_spec_slit_error = sPAlfs.specficPA_line_fluxes(
        flux, flux_error, line_names, line_ext_error, lines_available, lines_radial, param_estimated, param_requered, param_mod_name, param_model_values)

#######################################################################################################
# use the derived fluxes and ratios as input to Pyneb and calculate paramteres such as
# Te, Ne, ionic and chemical abudnances,  and ICFs
#######################################################################################################
    Te_slits, Ne_slits = TeNeslits.TeNe(flux_spec_slit, flux_spec_slit_error, line_names,
                                        lines_available, param_estimated, param_requered, param_mod_name, param_model_values)
...
if param_requered[index] == "yes":
    ...
    if param_requered[index] == "no":
        ...
        Te_PA, Ne_PA = dTeNenullspaxelss.nullTeNe(
            param_mod_name, param_model_values)
    ...
    a2Ds.analysis2D(flux, flux_error, flux_angles_norm, angles, line_names, line_ext_error, lines_available, lines_radial, param_estimated, param_requered,
                    param_mod_name, param_model_values, DD_name, DD_avail, DDxmin, DDxmax, DDymin, DDymax, hdr, flux_spec_slit_norm, flux_spec_slit_norm_error)

#######################################################################################################
# use the derived fluxes and ratios as input to Pyneb and calculate paramteres such as
# Te, Ne, ionic and chemical abudnances,  and ICFs
#######################################################################################################
    Te_2D, Ne_2D = TeNe2Ds.TeNe(flux, flux_error, line_names, line_ext_error, lines_available,
                                param_mod_name, param_model_values, param_estimated, param_requered, hdr, Te_PA, Ne_PA)
...
if param_requered[index] == "yes":
    mya2Ds.analysis2D(flux, flux_angles_norm, angles,
                      param_mod_name, param_model_values)
...
if param_requered[index] == "yes":
    pps.analysiswithPyNeb(flux, flux_angles, flux_spec_slit, angles, line_names, lines_available,
                          lines_radial, param_estimated, param_requered, param_mod_name, param_model_values, hdr)
