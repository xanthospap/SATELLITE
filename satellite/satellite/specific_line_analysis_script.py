...
def analysis_specific_line(flux, flux_err, angle, haa, hbb, widthslit,
                           lengthslit, x_CS_old, y_CS_old, x_slit_old,
                           y_slit_old, param_estimated, param_requered,
                           num_slit):
    ...
    centerx = (size_rotx) / 2 + (xoffset * math.cos(
        (-1) * math.pi * angle / 180) - yoffset * math.sin(
            (-1) * math.pi * angle / 180))
    centery = (size_roty) / 2 + (xoffset * math.sin(
        (-1) * math.pi * angle / 180) + yoffset * math.cos(
            (-1) * math.pi * angle / 180))
    ...
    if param_requered[index] == "yes":
        ...
        rect1 = patches.Rectangle(
            (centerx - 1 - (widthslit / 2), centery - (lengthslit / 2)),
            (widthslit / pixels_conversion) - 1,
            lengthslit / pixels_conversion,
            linewidth=1,
            edgecolor='k',
            facecolor='none')
        ax.add_patch(rect1)
...
#               plt.show()
    ...
    for i in range(centerx_new, centerx_new + lengthslit_new):
        if (widthslit_new % 2) == 0:
            for j in range(centery_new - widthslit_new // 2,
                           centery_new + widthslit_new // 2):
            ...
            for j in range(centery_new - (widthslit_new - 1) // 2,
                           centery_new + (widthslit_new - 1) // 2 + 1):
                if (flux_rot[i, j] == 0):
                    sum = sum
                if flux_rot[i, j] > 0 and ha_rot[
                        i, j] >= hb_rot[i, j] * 2.86 and hb_rot[i, j] > 0:
