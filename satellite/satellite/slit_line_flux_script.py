...
def slitfluxline(flux, flux_err, angle, haa, hbb, widthslit, lengthslit,
                 max_width):
    ...
    if widthslit < max_width and lengthslit <= max_width:
        k = 0
        sum = sumerror = 0.0
        for i in range(centerx, centerx +
                       lengthslit_new):  # i increases vertially from bottom
            if (widthslit_new % 2) == 0:
                for j in range(centery - widthslit_new // 2,
                               centery + widthslit_new //
                               2):  # j increases horizontally from left
                    ### Fluxes and errors
                    if (flux_rot[i, j] == 0):
                        sum = sum
                    if flux_rot[i, j] > 0 and ha_rot[
                            i, j] >= hb_rot[i, j] * 2.86 and hb_rot[i, j] > 0:
                        sum = sum + flux_rot[i, j]
                        sumerror = sumerror + flux_err_rot[i, j]**2
                        k = k + 1

            else:
                for j in range(centery - (widthslit_new - 1) // 2,
                               centery + (widthslit_new - 1) // 2 + 1):
                    if (flux_rot[i, j] == 0):
                        sum = sum
                    if flux_rot[i, j] > 0 and ha_rot[
                            i, j] >= hb_rot[i, j] * 2.86 and hb_rot[i, j] > 0:
    ...
    if widthslit > max_width or lengthslit > max_width:
        print(
            "Sorry, your slit width or/and length are larger that the true size of the image",
            file=file10)
    ...
    if widthslit == max_width and lengthslit == max_width:
        k = 0
        sum = 0.0
        for i in range(centerx - lengthslit // 2, centerx +
                       lengthslit // 2):  # i increases vertically from bottom
            for j in range(
                    centery - widthslit // 2, centery +
                    widthslit // 2):  # j increases horizontally from left
                if (flux_rot[i, j] == 0):
                    sum = sum
                if flux_rot[i, j] > 0 and ha_rot[
                        i, j] >= hb_rot[i, j] * 2.86 and hb_rot[i, j] > 0:
