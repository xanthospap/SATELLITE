...
def total_abundances_ICFs(ion_abun, ion_abun_error, atom_abun_fake, i, j,
                          line_names, lines_available, elem_abun_KB,
                          elem_ICF_KB, elem_abun_DIMS, elem_ICF_DIMS,
                          elem_abun_KB_error, elem_ICF_KB_error,
    ...
    if index1 == 1 and index2 == 0 and index3 == 0 and index4 == 0:
        ...
        O2_ab_er = np.sqrt(O2_ab_1_er**2 + O2_ab_2_er**2 + O2_ab_3_er**2 +
                           O2_ab_4_er**2) / 4.
    ...
    if (S3_ab == 0 and O2_ab != 0):
        ...
        a1 = a2 = a3 = 0
        a1 = (S2_ab * (0.433 * (O3_ab)**(-0.567) * (O2_ab)**(-0.433)) *
              O3_ab_er)**2
        a2 = (S2_ab * ((-0.433) * O3_ab**(0.433) * (O2_ab)**(-1.433)) *
              O2_ab_er)**2
    ...
    elem_abun_KB_error.He[i, j] = (He2_ab_er + He3_ab_er
                                  )  # Computes the He abundance

    if (He2_ab != 0):
        if (O2_ab != 0 or O3_ab != 0):
            elem_abun_KB.O[i, j] = icf.getElemAbundance(
                atom_abun,
                icf_list=['KB94_A10'
                         ]).get("KB94_A10")  # Computes the O abundance
            abunda = icf.getElemAbundance(atom_abun_fake, icf_list=[
                'KB94_A10'
            ]).get("KB94_A10")  # Computes the O abundance
            abunda[abunda == inf] = 0
            ...
            abunda = [
                i2 for i2 in abunda if i2 < elem_abun_KB.O[i, j] *
                3 and i2 > elem_abun_KB.O[i, j] * 0.3
            ]
            elem_abun_KB_error.O[i, j] = np.std(abunda)
            elem_ICF_KB.O[i, j] = elem_abun_KB.O[i, j] / (O2_ab + O3_ab)
            elem_ICF_KB_error.O[i, j] = elem_ICF_KB.O[i, j] * nfes.ratio_Error(
                elem_abun_KB.O[i, j], elem_abun_KB_error.O[i, j], O2_ab + O3_ab,
                O2_ab_er + O3_ab_er)

    if (elem_abun_KB.O[i, j] != 0 and O2_ab != 0 and N2_ab != 0):
        elem_abun_KB.N[i, j] = icf.getElemAbundance(
            atom_abun, icf_list=['KB94_A10', 'KB94_A1.10'
                                ]).get('KB94_A1.10')  # Computes the N abundance
        abunda = icf.getElemAbundance(
            atom_abun_fake,
            icf_list=['KB94_A10', 'KB94_A1.10'
                     ]).get('KB94_A1.10')  # Computes the N abundance
        abunda[abunda == inf] = 0
        abunda = np.nan_to_num(abunda)
        abunda = [i2 for i2 in abunda if i2 != 0]
        abunda = [
            i2 for i2 in abunda
            if i2 < elem_abun_KB.N[i, j] * 3 and i2 > elem_abun_KB.N[i, j] * 0.3
        ]
        elem_abun_KB_error.N[i, j] = np.std(abunda)
        elem_ICF_KB.N[i, j] = elem_abun_KB.N[i, j] / N2_ab
        elem_ICF_KB_error.N[i, j] = elem_ICF_KB.N[i, j] * nfes.ratio_Error(
            elem_abun_KB.N[i, j], elem_abun_KB_error.N[i, j], N2_ab, N2_ab_er)

    if (elem_abun_KB.O[i, j] != 0 and O2_ab != 0 and O3_ab != 0 and
            S2_ab != 0 and S3_ab != 0):
        elem_abun_KB.S[i, j] = icf.getElemAbundance(
            atom_abun,
            icf_list=['KB94_A10', 'KB94_A36.10'
                     ]).get("KB94_A36.10")  # Computes the S abundance
        abunda = icf.getElemAbundance(
            atom_abun_fake,
            icf_list=['KB94_A10', 'KB94_A36.10'
                     ]).get("KB94_A36.10")  # Computes the S abundance
        ...
        abunda = [
            i2 for i2 in abunda
            if i2 < elem_abun_KB.S[i, j] * 3 and i2 > elem_abun_KB.S[i, j] * 0.3
        ]
        elem_abun_KB_error.S[i, j] = np.std(abunda)
        elem_ICF_KB.S[i, j] = elem_abun_KB.S[i, j] / (S2_ab + S3_ab)
        elem_ICF_KB_error.S[i, j] = elem_ICF_KB.S[i, j] * nfes.ratio_Error(
            elem_abun_KB.S[i, j], elem_abun_KB_error.S[i, j], S2_ab + S3_ab,
            S2_ab_er + S3_ab_er)

    if (elem_abun_KB.O[i, j] != 0 and O2_ab != 0 and O3_ab != 0 and
            Ne3_ab != 0):
        elem_abun_KB.Ne[i, j] = icf.getElemAbundance(
            atom_abun,
            icf_list=['KB94_A10', 'KB94_A28.10'
                     ]).get("KB94_A28.10")  # Computes the Ne abundance
        abunda = icf.getElemAbundance(
            atom_abun_fake,
            icf_list=['KB94_A10', 'KB94_A28.10'
                     ]).get("KB94_A28.10")  # Computes the Ne abundance
        ...
        abunda = [i2 for i2 in abunda if i2 != 0]
        abunda = [
            i2 for i2 in abunda if i2 < elem_abun_KB.Ne[i, j] *
            3 and i2 > elem_abun_KB.Ne[i, j] * 0.3
        ]
        elem_abun_KB_error.Ne[i, j] = np.std(abunda)
        elem_ICF_KB.Ne[i, j] = elem_abun_KB.Ne[i, j] / (Ne3_ab)
        elem_ICF_KB_error.Ne[i, j] = elem_ICF_KB.Ne[i, j] * nfes.ratio_Error(
            elem_abun_KB.Ne[i, j], elem_abun_KB_error.Ne[i, j], Ne3_ab,
            Ne3_ab_er)

    if (Ar3_ab == 0 and Ar4_ab != 0):
        if (elem_abun_KB.Ne[i, j] != 0 and Ne3_ab != 0):
            elem_abun_KB.Ar[i, j] = icf.getElemAbundance(
                atom_abun,
                icf_list=['KB94_A10', 'KB94_A34'
                         ]).get("KB94_A34")  # Computes the Ar abundance
            abunda = icf.getElemAbundance(
                atom_abun_fake,
                icf_list=['KB94_A10', 'KB94_A34'
                         ]).get("KB94_A34")  # Computes the Ar abundance
            ...
            abunda = [i2 for i2 in abunda if i2 != 0]
            abunda = [
                i2 for i2 in abunda if i2 < elem_abun_KB.Ar[i, j] *
                3 and i2 > elem_abun_KB.Ar[i, j] * 0.3
            ]
            elem_abun_KB_error.Ar[i, j] = np.std(abunda)
            elem_ICF_KB.Ar[i, j] = elem_abun_KB.Ar[i, j] / (Ar4_ab)
            elem_ICF_KB_error.Ar[i,
                                 j] = elem_ICF_KB.Ar[i, j] * nfes.ratio_Error(
                                     elem_abun_KB.Ar[i, j],
                                     elem_abun_KB_error.Ar[i, j], Ar4_ab,
                                     Ar4_ab_er)
    if (Ar3_ab != 0 and Ar4_ab == 0):
        elem_abun_KB.Ar[i, j] = icf.getElemAbundance(
            atom_abun,
            icf_list=['KB94_A10',
                      'KB94_A32']).get("KB94_A32")  # Computes the Ar abundance
        abunda = icf.getElemAbundance(
            atom_abun_fake,
            icf_list=['KB94_A10',
                      'KB94_A32']).get("KB94_A32")  # Computes the Ar abundance
    ...
    if elem_abun_KB.N[i, j] != 0 and elem_abun_KB.O[i, j] != 0:
        elem_abun_KB_ratio.NO[i,
                              j] = elem_abun_KB.N[i, j] / elem_abun_KB.O[i, j]
        elem_abun_KB_ratio_error.NO[
            i, j] = elem_abun_KB_ratio.NO[i, j] * np.sqrt(
                (elem_abun_KB_error.N[i, j] / elem_abun_KB.N[i, j])**2 +
                (elem_abun_KB_error.O[i, j] / elem_abun_KB.O[i, j])**2)
    if elem_abun_KB.S[i, j] != 0 and elem_abun_KB.O[i, j] != 0:
        elem_abun_KB_ratio.SO[i,
                              j] = elem_abun_KB.S[i, j] / elem_abun_KB.O[i, j]
        elem_abun_KB_ratio_error.SO[
            i, j] = elem_abun_KB_ratio.SO[i, j] * np.sqrt(
                (elem_abun_KB_error.S[i, j] / elem_abun_KB.S[i, j])**2 +
                (elem_abun_KB_error.O[i, j] / elem_abun_KB.O[i, j])**2)
    if elem_abun_KB.Ne[i, j] != 0 and elem_abun_KB.O[i, j] != 0:
        elem_abun_KB_ratio.NeO[i,
                               j] = elem_abun_KB.Ne[i, j] / elem_abun_KB.O[i, j]
        elem_abun_KB_ratio_error.NeO[
            i, j] = elem_abun_KB_ratio.NeO[i, j] * np.sqrt(
                (elem_abun_KB_error.Ne[i, j] / elem_abun_KB.Ne[i, j])**2 +
                (elem_abun_KB_error.O[i, j] / elem_abun_KB.O[i, j])**2)
    if elem_abun_KB.Cl[i, j] != 0 and elem_abun_KB.O[i, j] != 0:
        elem_abun_KB_ratio.ClO[i,
                               j] = elem_abun_KB.Cl[i, j] / elem_abun_KB.O[i, j]
        elem_abun_KB_ratio_error.ClO[
            i, j] = elem_abun_KB_ratio.ClO[i, j] * np.sqrt(
                (elem_abun_KB_error.Cl[i, j] / elem_abun_KB.Cl[i, j])**2 +
                (elem_abun_KB_error.O[i, j] / elem_abun_KB.O[i, j])**2)
    if elem_abun_KB.Ar[i, j] != 0 and elem_abun_KB.O[i, j] != 0:
        elem_abun_KB_ratio.ArO[i,
                               j] = elem_abun_KB.Ar[i, j] / elem_abun_KB.O[i, j]
        elem_abun_KB_ratio_error.ArO[
            i, j] = elem_abun_KB_ratio.ArO[i, j] * np.sqrt(
                (elem_abun_KB_error.Ar[i, j] / elem_abun_KB.Ar[i, j])**2 +
                (elem_abun_KB_error.O[i, j] / elem_abun_KB.O[i, j])**2)
    ...
    elem_abun_DIMS_error.He[i, j] = (He2_ab_er + He3_ab_er
                                    )  # Computes the He abundance

    if ((He2_ab + He3_ab) != 0 and (O2_ab + O3_ab) != 0):
        if (O2_ab != 0 or O3_ab != 0):
            if ((He3_ab / (He2_ab + He3_ab)) < 0.95):
                elem_abun_DIMS.O[i, j] = icf.getElemAbundance(
                    atom_abun,
                    icf_list=['DIMS14_12'
                             ]).get("DIMS14_12")  # Computes the O abundance
                abunda = icf.getElemAbundance(
                    atom_abun_fake,
                    icf_list=['DIMS14_12'
                             ]).get("DIMS14_12")  # Computes the O abundance
                ...
                abunda = [i2 for i2 in abunda if i2 != 0]
                abunda = [
                    i2 for i2 in abunda if i2 < elem_abun_DIMS.O[i, j] *
                    3 and i2 > elem_abun_DIMS.O[i, j] * 0.3
                ]
                elem_abun_DIMS_error.O[i, j] = np.std(abunda)
                elem_ICF_DIMS.O[i, j] = elem_abun_DIMS.O[i, j] / (O2_ab + O3_ab)
                elem_ICF_DIMS_error.O[
                    i, j] = elem_ICF_DIMS.O[i, j] * nfes.ratio_Error(
                        elem_abun_DIMS.O[i, j], elem_abun_DIMS_error.O[i, j],
                        O2_ab + O3_ab, O2_ab_er + O3_ab_er)

    if (elem_abun_DIMS.O[i, j] != 0 and N2_ab != 0):
        if ((He2_ab + He3_ab) != 0 and (O2_ab + O3_ab) != 0):
            if (O2_ab != 0 or O3_ab != 0):
                if (He3_ab != 0 and (O3_ab / (O2_ab + O3_ab)) < 0.96):
                    elem_abun_DIMS.N[i, j] = icf.getElemAbundance(
                        atom_abun,
                        icf_list=['DIMS14_12', 'DIMS14_14'
                                 ]).get("DIMS14_14")  # Computes the N abundance
                    abunda = icf.getElemAbundance(
                        atom_abun_fake,
                        icf_list=['DIMS14_12', 'DIMS14_14'
                                 ]).get("DIMS14_14")  # Computes the N abundance
                    ...
                    abunda = [i2 for i2 in abunda if i2 != 0]
                    abunda = [
                        i2 for i2 in abunda if i2 < elem_abun_DIMS.N[i, j] *
                        3 and i2 > elem_abun_DIMS.N[i, j] * 0.3
                    ]
                    elem_abun_DIMS_error.N[i, j] = np.std(abunda)
                    elem_ICF_DIMS.N[i, j] = elem_abun_DIMS.N[i, j] / (N2_ab)
                    elem_ICF_DIMS_error.N[
                        i, j] = elem_ICF_DIMS.N[i, j] * nfes.ratio_Error(
                            elem_abun_DIMS.N[i, j],
                            elem_abun_DIMS_error.N[i, j], N2_ab, N2_ab_er)

                if (He3_ab == 0):
                    elem_abun_DIMS.N[i, j] = icf.getElemAbundance(
                        atom_abun, icf_list=['DIMS14_12', 'DIMS14_14b']).get(
                            "DIMS14_14b")  # Computes the N abundance
                    abunda = icf.getElemAbundance(
                        atom_abun_fake,
                        icf_list=['DIMS14_12', 'DIMS14_14b']).get(
                            "DIMS14_14b")  # Computes the N abundance
                    abunda[abunda == inf] = 0
                    abunda = np.nan_to_num(abunda)
                    abunda = [i2 for i2 in abunda if i2 != 0]
    ...
    if (elem_abun_DIMS.O[i, j] != 0):
        if ((He2_ab + He3_ab) != 0 and (O2_ab + O3_ab) != 0):
            if (O2_ab != 0 or O3_ab != 0):
                if (S2_ab != 0 and S3_ab_for_DIMS == -1 and
                    (O3_ab / (O2_ab + O3_ab)) < 0.95):
                    elem_abun_DIMS.S[i, j] = icf.getElemAbundance(
                        atom_abun,
                        icf_list=['DIMS14_12', 'DIMS14_26'
                                 ]).get("DIMS14_26")  # Computes the S abundance
                    abunda = icf.getElemAbundance(
                        atom_abun_fake,
                        icf_list=['DIMS14_12', 'DIMS14_26'
                                 ]).get("DIMS14_26")  # Computes the S abundance
                    ...
                    abunda = [i2 for i2 in abunda if i2 != 0]
                    abunda = [
                        i2 for i2 in abunda if i2 < elem_abun_DIMS.S[i, j] *
                        3 and i2 > elem_abun_DIMS.S[i, j] * 0.3
                    ]
                    elem_abun_DIMS_error.S[i, j] = np.std(abunda)
                    elem_ICF_DIMS.S[i,
                                    j] = elem_abun_DIMS.S[i,
                                                          j] / (S2_ab + S3_ab)
                    elem_ICF_DIMS_error.S[
                        i, j] = elem_ICF_DIMS.S[i, j] * nfes.ratio_Error(
                            elem_abun_DIMS.S[i, j], elem_abun_DIMS_error.S[i,
                                                                           j],
                            S2_ab + S3_ab, S2_ab_er + S3_ab_er)

                if (S2_ab != 0 and S3_ab_for_DIMS == 0 and
                    (He3_ab / (He2_ab + He3_ab)) > 0.02):
                    elem_abun_DIMS.S[i, j] = icf.getElemAbundance(
                        atom_abun_for_DIMS_S,
                        icf_list=['DIMS14_12', 'DIMS14_23'
                                 ]).get("DIMS14_23")  # Computes the S abundance
                    abunda = icf.getElemAbundance(
                        atom_abun_fake,
                        icf_list=['DIMS14_12', 'DIMS14_23'
                                 ]).get("DIMS14_23")  # Computes the S abundance
                    ...
                    elem_ICF_DIMS_error.S[
                        i, j] = elem_ICF_DIMS.S[i, j] * nfes.ratio_Error(
                            elem_abun_DIMS.S[i, j],
                            elem_abun_DIMS_error.S[i, j], S2_ab, S2_ab_er)

    if (elem_abun_DIMS.O[i, j] != 0):
        if (O2_ab != 0 or O3_ab != 0):
            if (Ne3_ab != 0 and (O3_ab / (O2_ab + O3_ab)) > 0.1):
                if (He3_ab == 0 and (O2_ab + O3_ab) != 0):
                    elem_abun_DIMS.Ne[i, j] = icf.getElemAbundance(
                        atom_abun, icf_list=['DIMS14_12', 'DIMS14_17b']).get(
                            "DIMS14_17b")  # Computes the Ne abundance
                    abunda = icf.getElemAbundance(
                        atom_abun_fake,
                        icf_list=['DIMS14_12', 'DIMS14_17b']).get(
                            "DIMS14_17b")  # Computes the Ne abundance
                    abunda[abunda == inf] = 0
                    abunda = np.nan_to_num(abunda)
                    abunda = [i2 for i2 in abunda if i2 != 0]
                    abunda = [
                        i2 for i2 in abunda if i2 < elem_abun_DIMS.Ne[i, j] *
                        3 and i2 > elem_abun_DIMS.Ne[i, j] * 0.3
                    ]
                    elem_abun_DIMS_error.Ne[i, j] = np.std(abunda)
                    elem_ICF_DIMS.Ne[i, j] = elem_abun_DIMS.Ne[i, j] / (Ne3_ab)
                    elem_ICF_DIMS_error.Ne[
                        i, j] = elem_ICF_DIMS.Ne[i, j] * nfes.ratio_Error(
                            elem_abun_DIMS.Ne[i, j],
                            elem_abun_DIMS_error.Ne[i, j], Ne3_ab, Ne3_ab_er)
                if (He3_ab != 0 and (O2_ab + O3_ab) != 0 and
                        He3_ab / (He2_ab + He3_ab) < 0.015):
                    elem_abun_DIMS.Ne[i, j] = icf.getElemAbundance(
                        atom_abun, icf_list=['DIMS14_12', 'DIMS14_17c']).get(
                            "DIMS14_17c")  # Computes the Ne abundance
                    abunda = icf.getElemAbundance(
                        atom_abun_fake,
                        icf_list=['DIMS14_12', 'DIMS14_17c']).get(
                            "DIMS14_17c")  # Computes the Ne abundance
                    abunda[abunda == inf] = 0
                    abunda = np.nan_to_num(abunda)
                    abunda = [i2 for i2 in abunda if i2 != 0]
                ...
                if (He3_ab != 0 and (O2_ab + O3_ab) != 0 and
                        He3_ab / (He2_ab + He3_ab) > 0.015):
                    elem_abun_DIMS.Ne[i, j] = icf.getElemAbundance(
                        atom_abun, icf_list=['DIMS14_12', 'DIMS14_17a']).get(
                            "DIMS14_17a")  # Computes the Ne abundance
                    abunda = icf.getElemAbundance(
                        atom_abun_fake,
                        icf_list=['DIMS14_12', 'DIMS14_17a']).get(
                            "DIMS14_17a")  # Computes the Ne abundance
                    abunda[abunda == inf] = 0
                    abunda = np.nan_to_num(abunda)
                    abunda = [i2 for i2 in abunda if i2 != 0]
    ...
    if (elem_abun_DIMS.O[i, j] != 0):
        if ((He2_ab + He3_ab) != 0 and (O2_ab + O3_ab) != 0):
            if (O2_ab != 0 or O3_ab != 0):
                if (Ar3_ab != 0):
                    if ((O3_ab / (O2_ab + O3_ab)) <= 0.5 and
                        (O3_ab / (O2_ab + O3_ab)) > 0.05):
                        elem_abun_DIMS.Ar[i, j] = icf.getElemAbundance(
                            atom_abun, icf_list=['DIMS14_12', 'DIMS14_35']).get(
                                "DIMS14_35")  # Computes the Ar abundance
                        abunda = icf.getElemAbundance(
                            atom_abun_fake,
                            icf_list=['DIMS14_12', 'DIMS14_35']).get(
                                "DIMS14_35")  # Computes the Ar abundance
                        abunda[abunda == inf] = 0
                        abunda = np.nan_to_num(abunda)
                        abunda = [i2 for i2 in abunda if i2 != 0]
                        abunda = [
                            i2 for i2 in abunda
                            if i2 < elem_abun_DIMS.Ar[i, j] *
                            3 and i2 > elem_abun_DIMS.Ar[i, j] * 0.3
                        ]
                        elem_abun_DIMS_error.Ar[i, j] = np.std(abunda)
                        elem_ICF_DIMS.Ar[i,
                                         j] = elem_abun_DIMS.Ar[i, j] / (Ar3_ab)
                        elem_ICF_DIMS_error.Ar[
                            i, j] = elem_ICF_DIMS.Ar[i, j] * nfes.ratio_Error(
                                elem_abun_DIMS.Ar[i, j],
                                elem_abun_DIMS_error.Ar[i,
                                                        j], Ar3_ab, Ar3_ab_er)

                    if ((O3_ab / (O2_ab + O3_ab)) < 0.95 and
                        (O3_ab / (O2_ab + O3_ab)) > 0.5):
                        elem_abun_DIMS.Ar[i, j] = icf.getElemAbundance(
                            atom_abun, icf_list=['DIMS14_12', 'DIMS14_36']).get(
                                "DIMS14_36")  # Computes the Ar abundance
                        abunda = icf.getElemAbundance(
                            atom_abun_fake,
                            icf_list=['DIMS14_12', 'DIMS14_36']).get(
                                "DIMS14_36")  # Computes the Ar abundance
                        abunda[abunda == inf] = 0
                        abunda = np.nan_to_num(abunda)
                        abunda = [i2 for i2 in abunda if i2 != 0]
        ...
        if ((He2_ab + He3_ab) != 0 and (O2_ab + O3_ab) != 0):
            if (O2_ab != 0 or O3_ab != 0):
                if (Cl3_ab != 0 and Cl2_ab == 0 and Cl4_ab == 0):
                    if ((He3_ab / (He2_ab + He3_ab)) > 0.02 and
                        (He3_ab / (He2_ab + He3_ab)) < 0.95):
                        elem_abun_DIMS.Cl[i, j] = icf.getElemAbundance(
                            atom_abun, icf_list=['DIMS14_12', 'DIMS14_29']).get(
                                "DIMS14_29")  # Computes the Ar abundance
                        abunda = icf.getElemAbundance(
                            atom_abun_fake,
                            icf_list=['DIMS14_12', 'DIMS14_29']).get(
                                "DIMS14_29")  # Computes the Ar abundance
                        abunda[abunda == inf] = 0
                        abunda = np.nan_to_num(abunda)
                        abunda = [i2 for i2 in abunda if i2 != 0]
                        abunda = [
                            i2 for i2 in abunda
                            if i2 < elem_abun_DIMS.Cl[i, j] *
                            3 and i2 > elem_abun_DIMS.Cl[i, j] * 0.3
                        ]
                        elem_abun_DIMS_error.Cl[i, j] = np.std(abunda)
                        elem_ICF_DIMS.Cl[i,
                                         j] = elem_abun_DIMS.Cl[i, j] / (Cl3_ab)
                        elem_ICF_DIMS_error.Cl[
                            i, j] = elem_ICF_DIMS.Cl[i, j] * nfes.ratio_Error(
                                elem_abun_DIMS.Cl[i, j],
                                elem_abun_DIMS_error.Cl[i,
                                                        j], Cl3_ab, Cl3_ab_er)
                if (Cl3_ab != 0 and Cl2_ab != 0 and Cl4_ab == 0):
                    if ((He3_ab / (He2_ab + He3_ab)) <= 0.02):
                        elem_abun_DIMS.Cl[i, j] = icf.getElemAbundance(
                            atom_abun,
                            icf_list=['DIMS14_12', 'DIMS14_29b']).get(
                                "DIMS14_29b")  # Computes the Ar abundance
                        abunda = icf.getElemAbundance(
                            atom_abun_fake,
                            icf_list=['DIMS14_12', 'DIMS14_29b']).get(
                                "DIMS14_29b")  # Computes the Ar abundance
                        ...
                        elem_abun_DIMS_error.Cl[i, j] = np.std(abunda)
                        elem_ICF_DIMS.Cl[
                            i, j] = elem_abun_DIMS.Cl[i, j] / (Cl3_ab + Cl2_ab)
                        elem_ICF_DIMS_error.Cl[
                            i, j] = elem_ICF_DIMS.Cl[i, j] * nfes.ratio_Error(
                                elem_abun_DIMS.Cl[i, j],
                                elem_abun_DIMS_error.Cl[i, j], Cl3_ab + Cl2_ab,
                                Cl3_ab_er + Cl2_ab_er)
                if (Cl3_ab != 0 and Cl2_ab != 0 and Cl4_ab != 0):
                    if ((He3_ab / (He2_ab + He3_ab)) >= 0 and
                        (He3_ab / (He2_ab + He3_ab)) <= 1):
                        elem_abun_DIMS.Cl[i, j] = icf.getElemAbundance(
                            atom_abun, icf_list=['DIMS14_12', 'DIMS14_32']).get(
                                "DIMS14_32")  # Computes the Ar abundance
                        abunda = icf.getElemAbundance(
                            atom_abun_fake,
                            icf_list=['DIMS14_12', 'DIMS14_32']).get(
                                "DIMS14_32")  # Computes the Ar abundance
                        abunda[abunda == inf] = 0
                        abunda = np.nan_to_num(abunda)
                        abunda = [i2 for i2 in abunda if i2 != 0]
                        ...
                        elem_abun_DIMS_error.Cl[i, j] = np.std(abunda)
                        elem_ICF_DIMS.Cl[i, j] = elem_abun_DIMS.Cl[i, j] / (
                            Cl3_ab + Cl2_ab + Cl4_ab)
                        elem_ICF_DIMS_error.Cl[
                            i, j] = elem_ICF_DIMS.Cl[i, j] * nfes.ratio_Error(
                                elem_abun_DIMS.Cl[i, j],
                                elem_abun_DIMS_error.Cl[i, j], Cl3_ab + Cl2_ab +
                                Cl4_ab, Cl3_ab_er + Cl2_ab_er + Cl4_ab_er)

    if elem_abun_DIMS.N[i, j] != 0 and elem_abun_DIMS.O[i, j] != 0:
        elem_abun_DIMS_ratio.NO[
            i, j] = elem_abun_DIMS.N[i, j] / elem_abun_DIMS.O[i, j]
        elem_abun_DIMS_ratio_error.NO[
            i, j] = elem_abun_DIMS_ratio.NO[i, j] * np.sqrt(
                (elem_abun_DIMS_error.N[i, j] / elem_abun_DIMS.N[i, j])**2 +
                (elem_abun_DIMS_error.O[i, j] / elem_abun_DIMS.O[i, j])**2)
    if elem_abun_DIMS.S[i, j] != 0 and elem_abun_DIMS.O[i, j] != 0:
        elem_abun_DIMS_ratio.SO[
            i, j] = elem_abun_DIMS.S[i, j] / elem_abun_DIMS.O[i, j]
        elem_abun_DIMS_ratio_error.SO[
            i, j] = elem_abun_DIMS_ratio.SO[i, j] * np.sqrt(
                (elem_abun_DIMS_error.S[i, j] / elem_abun_DIMS.S[i, j])**2 +
                (elem_abun_DIMS_error.O[i, j] / elem_abun_DIMS.O[i, j])**2)
    if elem_abun_DIMS.Ne[i, j] != 0 and elem_abun_DIMS.O[i, j] != 0:
        elem_abun_DIMS_ratio.NeO[
            i, j] = elem_abun_DIMS.Ne[i, j] / elem_abun_DIMS.O[i, j]
        elem_abun_DIMS_ratio_error.NeO[
            i, j] = elem_abun_DIMS_ratio.NeO[i, j] * np.sqrt(
                (elem_abun_DIMS_error.Ne[i, j] / elem_abun_DIMS.Ne[i, j])**2 +
                (elem_abun_DIMS_error.O[i, j] / elem_abun_DIMS.O[i, j])**2)
    if elem_abun_DIMS.Cl[i, j] != 0 and elem_abun_DIMS.O[i, j] != 0:
        elem_abun_DIMS_ratio.ClO[
            i, j] = elem_abun_DIMS.Cl[i, j] / elem_abun_DIMS.O[i, j]
        elem_abun_DIMS_ratio_error.ClO[
            i, j] = elem_abun_DIMS_ratio.ClO[i, j] * np.sqrt(
                (elem_abun_DIMS_error.Cl[i, j] / elem_abun_DIMS.Cl[i, j])**2 +
                (elem_abun_DIMS_error.O[i, j] / elem_abun_DIMS.O[i, j])**2)
    if elem_abun_DIMS.Ar[i, j] != 0 and elem_abun_DIMS.O[i, j] != 0:
        elem_abun_DIMS_ratio.ArO[
            i, j] = elem_abun_DIMS.Ar[i, j] / elem_abun_DIMS.O[i, j]
        elem_abun_DIMS_ratio_error.ArO[
            i, j] = elem_abun_DIMS_ratio.ArO[i, j] * np.sqrt(
                (elem_abun_DIMS_error.Ar[i, j] / elem_abun_DIMS.Ar[i, j])**2 +
                (elem_abun_DIMS_error.O[i, j] / elem_abun_DIMS.O[i, j])**2)
