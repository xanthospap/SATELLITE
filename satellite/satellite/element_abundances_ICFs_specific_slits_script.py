...
def total_abundances_ICFs(ion_abun, ion_abun_error, atom_abun_fake, i,
    ...
    elem_abun_KB_error.He[i] = (He2_ab_er + He3_ab_er
                               )  # Computes the He abundance

    if (He2_ab != 0):
        if (O2_ab != 0 or O3_ab != 0):
            elem_abun_KB.O[i] = icf.getElemAbundance(
                atom_abun,
                icf_list=['KB94_A10'
            ...
            abunda = [
                i2 for i2 in abunda
                if i2 < elem_abun_KB.O[i] * 3 and i2 > elem_abun_KB.O[i] * 0.3
            ]
            elem_abun_KB_error.O[i] = np.std(abunda)
            elem_ICF_KB.O[i] = elem_abun_KB.O[i] / (O2_ab + O3_ab)
            elem_ICF_KB_error.O[i] = elem_ICF_KB.O[i] * nfes.ratio_Error(
                elem_abun_KB.O[i], elem_abun_KB_error.O[i], O2_ab + O3_ab,
                O2_ab_er + O3_ab_er)

    if (elem_abun_KB.O[i] != 0 and O2_ab != 0 and N2_ab != 0):
        elem_abun_KB.N[i] = icf.getElemAbundance(
        ...
        abunda = [
            i2 for i2 in abunda
            if i2 < elem_abun_KB.N[i] * 3 and i2 > elem_abun_KB.N[i] * 0.3
        ]
        elem_abun_KB_error.N[i] = np.std(abunda)
        elem_ICF_KB.N[i] = elem_abun_KB.N[i] / N2_ab
        elem_ICF_KB_error.N[i] = elem_ICF_KB.N[i] * nfes.ratio_Error(
            elem_abun_KB.N[i], elem_abun_KB_error.N[i], N2_ab, N2_ab_er)

    if (elem_abun_KB.O[i] != 0 and O2_ab != 0 and O3_ab != 0 and S2_ab != 0 and
            S3_ab != 0):
        elem_abun_KB.S[i] = icf.getElemAbundance(
            atom_abun,
        ...
        abunda = [
            i2 for i2 in abunda
            if i2 < elem_abun_KB.S[i] * 3 and i2 > elem_abun_KB.S[i] * 0.3
        ]
        elem_abun_KB_error.S[i] = np.std(abunda)
        elem_ICF_KB.S[i] = elem_abun_KB.S[i] / (S2_ab + S3_ab)
        elem_ICF_KB_error.S[i] = elem_ICF_KB.S[i] * nfes.ratio_Error(
            elem_abun_KB.S[i], elem_abun_KB_error.S[i], S2_ab + S3_ab,
            S2_ab_er + S3_ab_er)

    if (elem_abun_KB.O[i] != 0 and O2_ab != 0 and O3_ab != 0 and Ne3_ab != 0):
        elem_abun_KB.Ne[i] = icf.getElemAbundance(
            atom_abun,
        ...
        abunda = [
            i2 for i2 in abunda
            if i2 < elem_abun_KB.Ne[i] * 3 and i2 > elem_abun_KB.Ne[i] * 0.3
        ]
        elem_abun_KB_error.Ne[i] = np.std(abunda)
        elem_ICF_KB.Ne[i] = elem_abun_KB.Ne[i] / (Ne3_ab)
        elem_ICF_KB_error.Ne[i] = elem_ICF_KB.Ne[i] * nfes.ratio_Error(
            elem_abun_KB.Ne[i], elem_abun_KB_error.Ne[i], Ne3_ab, Ne3_ab_er)

    if (Ar3_ab == 0 and Ar4_ab != 0):
        if (elem_abun_KB.Ne[i] != 0 and Ne3_ab != 0):
            elem_abun_KB.Ar[i] = icf.getElemAbundance(
                atom_abun,
            ...
            abunda = [
                i2 for i2 in abunda
                if i2 < elem_abun_KB.Ar[i] * 3 and i2 > elem_abun_KB.Ar[i] * 0.3
            ]
            elem_abun_KB_error.Ar[i] = np.std(abunda)
            elem_ICF_KB.Ar[i] = elem_abun_KB.Ar[i] / (Ar4_ab)
            elem_ICF_KB_error.Ar[i] = elem_ICF_KB.Ar[i] * nfes.ratio_Error(
                elem_abun_KB.Ar[i], elem_abun_KB_error.Ar[i], Ar4_ab, Ar4_ab_er)
    if (Ar3_ab != 0 and Ar4_ab == 0):
        elem_abun_KB.Ar[i] = icf.getElemAbundance(
            atom_abun,
            icf_list=['KB94_A10',
        ...
        elem_ICF_KB_error.Ar[i] = elem_ICF_KB.Ar[i] * nfes.ratio_Error(
            elem_abun_KB.Ar[i], elem_abun_KB_error.Ar[i], Ar3_ab, Ar3_ab_er)

    if elem_abun_KB.N[i] != 0 and elem_abun_KB.O[i] != 0:
        elem_abun_KB_ratio.NO[i] = elem_abun_KB.N[i] / elem_abun_KB.O[i]
        elem_abun_KB_ratio_error.NO[i] = elem_abun_KB_ratio.NO[i] * np.sqrt(
            (elem_abun_KB_error.N[i] / elem_abun_KB.N[i])**2 +
            (elem_abun_KB_error.O[i] / elem_abun_KB.O[i])**2)
    if elem_abun_KB.S[i] != 0 and elem_abun_KB.O[i] != 0:
        elem_abun_KB_ratio.SO[i] = elem_abun_KB.S[i] / elem_abun_KB.O[i]
        elem_abun_KB_ratio_error.SO[i] = elem_abun_KB_ratio.SO[i] * np.sqrt(
            (elem_abun_KB_error.S[i] / elem_abun_KB.S[i])**2 +
            (elem_abun_KB_error.O[i] / elem_abun_KB.O[i])**2)
    if elem_abun_KB.Ne[i] != 0 and elem_abun_KB.O[i] != 0:
        elem_abun_KB_ratio.NeO[i] = elem_abun_KB.Ne[i] / elem_abun_KB.O[i]
        elem_abun_KB_ratio_error.NeO[i] = elem_abun_KB_ratio.NeO[i] * np.sqrt(
            (elem_abun_KB_error.Ne[i] / elem_abun_KB.Ne[i])**2 +
            (elem_abun_KB_error.O[i] / elem_abun_KB.O[i])**2)
    if elem_abun_KB.Cl[i] != 0 and elem_abun_KB.O[i] != 0:
        elem_abun_KB_ratio.ClO[i] = elem_abun_KB.Cl[i] / elem_abun_KB.O[i]
        elem_abun_KB_ratio_error.ClO[i] = elem_abun_KB_ratio.ClO[i] * np.sqrt(
            (elem_abun_KB_error.Cl[i] / elem_abun_KB.Cl[i])**2 +
            (elem_abun_KB_error.O[i] / elem_abun_KB.O[i])**2)
    if elem_abun_KB.Ar[i] != 0 and elem_abun_KB.O[i] != 0:
        elem_abun_KB_ratio.ArO[i] = elem_abun_KB.Ar[i] / elem_abun_KB.O[i]
        elem_abun_KB_ratio_error.ArO[i] = elem_abun_KB_ratio.ArO[i] * np.sqrt(
            (elem_abun_KB_error.Ar[i] / elem_abun_KB.Ar[i])**2 +
            (elem_abun_KB_error.O[i] / elem_abun_KB.O[i])**2)
    ...
    elem_abun_DIMS_error.He[i] = (He2_ab_er + He3_ab_er
                                 )  # Computes the He abundance

    if ((He2_ab + He3_ab) != 0 and (O2_ab + O3_ab) != 0):
        if (O2_ab != 0 or O3_ab != 0):
            if ((He3_ab / (He2_ab + He3_ab)) < 0.95):
                elem_abun_DIMS.O[i] = icf.getElemAbundance(
                    atom_abun,
                    icf_list=['DIMS14_12'
                ...
                abunda = [i2 for i2 in abunda if i2 != 0]
                abunda = [
                    i2 for i2 in abunda if i2 < elem_abun_DIMS.O[i] *
                    3 and i2 > elem_abun_DIMS.O[i] * 0.3
                ]
                elem_abun_DIMS_error.O[i] = np.std(abunda)
                elem_ICF_DIMS.O[i] = elem_abun_DIMS.O[i] / (O2_ab + O3_ab)
                elem_ICF_DIMS_error.O[
                    i] = elem_ICF_DIMS.O[i] * nfes.ratio_Error(
                        elem_abun_DIMS.O[i], elem_abun_DIMS_error.O[i],
        ...
        if ((He2_ab + He3_ab) != 0 and (O2_ab + O3_ab) != 0):
            if (O2_ab != 0 or O3_ab != 0):
                if (He3_ab != 0 and (O3_ab / (O2_ab + O3_ab)) < 0.96):
                    elem_abun_DIMS.N[i] = icf.getElemAbundance(
                        atom_abun,
                    ...
                    abunda = [i2 for i2 in abunda if i2 != 0]
                    abunda = [
                        i2 for i2 in abunda if i2 < elem_abun_DIMS.N[i] *
                        3 and i2 > elem_abun_DIMS.N[i] * 0.3
                    ]
                    elem_abun_DIMS_error.N[i] = np.std(abunda)
                    elem_ICF_DIMS.N[i] = elem_abun_DIMS.N[i] / (N2_ab)
                    elem_ICF_DIMS_error.N[
                        i] = elem_ICF_DIMS.N[i] * nfes.ratio_Error(
                            elem_abun_DIMS.N[i], elem_abun_DIMS_error.N[i],
                            N2_ab, N2_ab_er)
        ...
        if ((He2_ab + He3_ab) != 0 and (O2_ab + O3_ab) != 0):
            if (O2_ab != 0 or O3_ab != 0):
                if (S2_ab != 0 and S3_ab_for_DIMS == -1 and
                    ...
                    elem_abun_DIMS.S[i] = icf.getElemAbundance(
                        atom_abun,
                    ...
                    abunda = [i2 for i2 in abunda if i2 != 0]
                    abunda = [
                        i2 for i2 in abunda if i2 < elem_abun_DIMS.S[i] *
                        3 and i2 > elem_abun_DIMS.S[i] * 0.3
                    ]
                    elem_abun_DIMS_error.S[i] = np.std(abunda)
                    elem_ICF_DIMS.S[i] = elem_abun_DIMS.S[i] / (S2_ab + S3_ab)
                    elem_ICF_DIMS_error.S[
                        i] = elem_ICF_DIMS.S[i] * nfes.ratio_Error(
                            elem_abun_DIMS.S[i], elem_abun_DIMS_error.S[i],
        ...
        if (O2_ab != 0 or O3_ab != 0):
            if (Ne3_ab != 0 and (O3_ab / (O2_ab + O3_ab)) > 0.1):
                if (He3_ab == 0 and (O2_ab + O3_ab) != 0):
                    elem_abun_DIMS.Ne[i] = icf.getElemAbundance(
                    ...
                    abunda = [i2 for i2 in abunda if i2 != 0]
                    abunda = [
                        i2 for i2 in abunda if i2 < elem_abun_DIMS.Ne[i] *
                        3 and i2 > elem_abun_DIMS.Ne[i] * 0.3
                    ]
                    elem_abun_DIMS_error.Ne[i] = np.std(abunda)
                    elem_ICF_DIMS.Ne[i] = elem_abun_DIMS.Ne[i] / (Ne3_ab)
                    elem_ICF_DIMS_error.Ne[
                        i] = elem_ICF_DIMS.Ne[i] * nfes.ratio_Error(
                            elem_abun_DIMS.Ne[i], elem_abun_DIMS_error.Ne[i],
                            Ne3_ab, Ne3_ab_er)
            ...
            if (O2_ab != 0 or O3_ab != 0):
                if (Ar3_ab != 0):
                    if ((O3_ab / (O2_ab + O3_ab)) <= 0.5 and
                        ...
                        elem_abun_DIMS.Ar[i] = icf.getElemAbundance(
                        ...
                        abunda = [i2 for i2 in abunda if i2 != 0]
                        abunda = [
                            i2 for i2 in abunda if i2 < elem_abun_DIMS.Ar[i] *
                            3 and i2 > elem_abun_DIMS.Ar[i] * 0.3
                        ]
                        elem_abun_DIMS_error.Ar[i] = np.std(abunda)
                        elem_ICF_DIMS.Ar[i] = elem_abun_DIMS.Ar[i] / (Ar3_ab)
                        elem_ICF_DIMS_error.Ar[
                            i] = elem_ICF_DIMS.Ar[i] * nfes.ratio_Error(
                                elem_abun_DIMS.Ar[i],
                                elem_abun_DIMS_error.Ar[i], Ar3_ab, Ar3_ab_er)
        ...
        if ((He2_ab + He3_ab) != 0 and (O2_ab + O3_ab) != 0):
            if (O2_ab != 0 or O3_ab != 0):
                if (Cl3_ab != 0 and Cl2_ab == 0 and Cl4_ab == 0):
                    if ((He3_ab / (He2_ab + He3_ab)) > 0.01 and
                        (He3_ab / (He2_ab + He3_ab)) < 0.95):
                        elem_abun_DIMS.Cl[i] = icf.getElemAbundance(
                        ...
                        abunda = [i2 for i2 in abunda if i2 != 0]
                        abunda = [
                            i2 for i2 in abunda if i2 < elem_abun_DIMS.Cl[i] *
                            3 and i2 > elem_abun_DIMS.Cl[i] * 0.3
                        ]
                        elem_abun_DIMS_error.Cl[i] = np.std(abunda)
                        elem_ICF_DIMS.Cl[i] = elem_abun_DIMS.Cl[i] / (Cl3_ab)
                        elem_ICF_DIMS_error.Cl[
                            i] = elem_ICF_DIMS.Cl[i] * nfes.ratio_Error(
                                elem_abun_DIMS.Cl[i],
                                elem_abun_DIMS_error.Cl[i], Cl3_ab, Cl3_ab_er)
                if (Cl3_ab != 0 and Cl2_ab != 0 and Cl4_ab == 0):
                    if ((He3_ab / (He2_ab + He3_ab)) <= 0.02):
                        ...
                        elem_ICF_DIMS_error.Cl[
                            i] = elem_ICF_DIMS.Cl[i] * nfes.ratio_Error(
                                elem_abun_DIMS.Cl[i],
                                elem_abun_DIMS_error.Cl[i], Cl3_ab + Cl2_ab,
                                Cl3_ab_er + Cl2_ab_er)
                if (Cl3_ab != 0 and Cl2_ab != 0 and Cl4_ab != 0):
                    if ((He3_ab / (He2_ab + He3_ab)) >= 0 and
                        ...
                        elem_abun_DIMS_error.Cl[i] = np.std(abunda)
                        elem_ICF_DIMS.Cl[i] = elem_abun_DIMS.Cl[i] / (
    ...
    if elem_abun_DIMS.N[i] != 0 and elem_abun_DIMS.O[i] != 0:
        ...
        elem_abun_DIMS_ratio_error.NO[i] = elem_abun_DIMS_ratio.NO[i] * np.sqrt(
            (elem_abun_DIMS_error.N[i] / elem_abun_DIMS.N[i])**2 +
            (elem_abun_DIMS_error.O[i] / elem_abun_DIMS.O[i])**2)
    if elem_abun_DIMS.S[i] != 0 and elem_abun_DIMS.O[i] != 0:
        elem_abun_DIMS_ratio.SO[i] = elem_abun_DIMS.S[i] / elem_abun_DIMS.O[i]
        elem_abun_DIMS_ratio_error.SO[i] = elem_abun_DIMS_ratio.SO[i] * np.sqrt(
            (elem_abun_DIMS_error.S[i] / elem_abun_DIMS.S[i])**2 +
            (elem_abun_DIMS_error.O[i] / elem_abun_DIMS.O[i])**2)
    if elem_abun_DIMS.Ne[i] != 0 and elem_abun_DIMS.O[i] != 0:
        ...
        elem_abun_DIMS_ratio_error.NeO[
            i] = elem_abun_DIMS_ratio.NeO[i] * np.sqrt(
                (elem_abun_DIMS_error.Ne[i] / elem_abun_DIMS.Ne[i])**2 +
                (elem_abun_DIMS_error.O[i] / elem_abun_DIMS.O[i])**2)
    if elem_abun_DIMS.Cl[i] != 0 and elem_abun_DIMS.O[i] != 0:
        ...
        elem_abun_DIMS_ratio_error.ClO[
            i] = elem_abun_DIMS_ratio.ClO[i] * np.sqrt(
                (elem_abun_DIMS_error.Cl[i] / elem_abun_DIMS.Cl[i])**2 +
                (elem_abun_DIMS_error.O[i] / elem_abun_DIMS.O[i])**2)
    if elem_abun_DIMS.Ar[i] != 0 and elem_abun_DIMS.O[i] != 0:
        ...
        elem_abun_DIMS_ratio_error.ArO[
            i] = elem_abun_DIMS_ratio.ArO[i] * np.sqrt(
                (elem_abun_DIMS_error.Ar[i] / elem_abun_DIMS.Ar[i])**2 +
                (elem_abun_DIMS_error.O[i] / elem_abun_DIMS.O[i])**2)
