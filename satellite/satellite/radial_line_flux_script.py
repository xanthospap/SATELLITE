...
def radialfluxline(flux, flux_err, angle, haa, hbb, widthslit, lengthslit,
                   max_width, pix_sc):
    ...
    if widthslit < max_width and lengthslit <= max_width:
        ...
        for i in range(centerx,
                       centerx + lengthslit_new):  # i anebainei pros ta panw
            sum = sumerror = 0.0
            if (widthslit_new % 2) == 0:
                for j in range(centery - widthslit_new // 2, centery +
                               widthslit_new // 2):  # to j einai orizontia
                    if (flux_rot[i, j] == 0):
                        sum = sum
                ...
                for j in range(centery - (widthslit_new - 1) // 2,
                               centery + (widthslit_new - 1) // 2 +
                               1):  # to j einai orizontia
                    if (flux_rot[i, j] == 0):
                        sum = sum
                    if flux_rot[i, j] > 0 and ha_rot[
                            i, j] >= hb_rot[i, j] * 2.86 and hb_rot[i, j] > 0:
