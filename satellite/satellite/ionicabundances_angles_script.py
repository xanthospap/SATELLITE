...
def ionicabundances(obs, obs_fake, Te, Te_error, Te_fake, Ne, Ne_error, Ne_fake,
                    i, ang, line_names, lines_available, ion_abun,
                    ion_abun_error, param_estimated, param_requered,
                    param_mod_name):
    ...
    if lines_available[index_line] == "yes":
        He1a_abund_fake = He1.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['He1r_5876A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(5876)',
            Hbeta=100.0)
        He1a_abund = He1.getIonAbundance(
            int_ratio=obs.getIntens(0)['He1r_5876A'],
            tem=Teinput,
            den=Neinput,
            to_eval='L(5876)',
            Hbeta=100.0)
        ...
        He1a_abund_fake = [
            i2 for i2 in He1a_abund_fake
            if i2 < He1a_abund * 3 and i2 > He1a_abund * 0.3
        ]
    ...
    if lines_available[index_line] == "yes":
        He1b_abund_fake = He1.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['He1r_6678A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(6678)',
            Hbeta=100.0)
        He1b_abund = He1.getIonAbundance(
            int_ratio=obs.getIntens(0)['He1r_6678A'],
            tem=Teinput,
            den=Neinput,
            to_eval='L(6678)',
            Hbeta=100.0)
        ...
        He1b_abund_fake = [
            i2 for i2 in He1b_abund_fake
            if i2 < He1b_abund * 3 and i2 > He1b_abund * 0.3
        ]
    ...
    if np.isnan(He1a_abund_fake) == True:
        He1a_abund_fake = 0
    ion_abun_error.HeIa[i] = He1a_abund_fake
    if np.isnan(He1b_abund_fake) == True:
        He1b_abund_fake = 0
    ion_abun_error.HeIb[i] = He1b_abund_fake

    print('He+(5786)/H+ = {:5.2e}'.format(ion_abun.HeIa[i]),
          'He+(6678)/H+ = {:5.2e}'.format(ion_abun.HeIb[i]),
          file=file10)
    print('error He+(5786)/H+ = {:5.2e}'.format(ion_abun_error.HeIa[i]),
          'He+(6678)/H+ = {:5.2e}'.format(ion_abun_error.HeIb[i]),
          file=file10)
    #############################################################
    ...
    if lines_available[index_line] == "yes":
        He2a_abund_fake = He2.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['He2r_4686A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(4686)',
            Hbeta=100.0)
        He2a_abund = He2.getIonAbundance(
            int_ratio=obs.getIntens(0)['He2r_4686A'],
            tem=Teinput,
            den=Neinput,
            to_eval='L(4686)',
            Hbeta=100.0)
        ...
        He2a_abund_fake = [
            i2 for i2 in He2a_abund_fake
            if i2 < He2a_abund * 3 and i2 > He2a_abund * 0.3
        ]
    ...
    if lines_available[index_line] == "yes":
        He2b_abund_fake = He2.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['He2r_5411A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(5411)',
            Hbeta=100.0)
        He2b_abund = He2.getIonAbundance(
            int_ratio=obs.getIntens(0)['He2r_5411A'],
            tem=Teinput,
            den=Neinput,
            to_eval='L(5411)',
            Hbeta=100.0)
        ...
        He2b_abund_fake = [
            i2 for i2 in He2b_abund_fake
            if i2 < He2b_abund * 3 and i2 > He2b_abund * 0.3
        ]
    ...
    if np.isnan(He2a_abund_fake) == True:
        He2a_abund_fake = 0
    ion_abun_error.HeIIa[i] = He2a_abund_fake
    if np.isnan(He2b_abund_fake) == True:
        He2b_abund_fake = 0
    ion_abun_error.HeIIb[i] = He2b_abund_fake

    print('He+2(4686)/H+ = {:5.2e}'.format(ion_abun.HeIIa[i]),
          'He+2(5412)/H+ = {:5.2e}'.format(ion_abun.HeIIb[i]),
          file=file10)
    print('He+2(4686)/H+ = {:5.2e}'.format(ion_abun_error.HeIIa[i]),
          'He+2(5412)/H+ = {:5.2e}'.format(ion_abun_error.HeIIb[i]),
          file=file10)
    #############################################################
    ...
    if lines_available[index_line] == "yes":
        O1a_abund_fake = O1.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['O1_5577A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(5577)',
            Hbeta=100.0)
        O1a_abund = O1.getIonAbundance(int_ratio=obs.getIntens(0)['O1_5577A'],
                                       tem=Teinput,
                                       den=Neinput,
                                       to_eval='L(5577)',
                                       Hbeta=100.0)
        ...
        O1a_abund_fake = [
            i2 for i2 in O1a_abund_fake
            if i2 < O1a_abund * 3 and i2 > O1a_abund * 0.3
        ]
    ...
    if lines_available[index_line] == "yes":
        O1b_abund_fake = O1.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['O1_6300A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(6300)',
            Hbeta=100.0)
        O1b_abund = O1.getIonAbundance(int_ratio=obs.getIntens(0)['O1_6300A'],
                                       tem=Teinput,
                                       den=Neinput,
                                       to_eval='L(6300)',
                                       Hbeta=100.0)
        ...
        O1b_abund_fake = [
            i2 for i2 in O1b_abund_fake
            if i2 < O1b_abund * 3 and i2 > O1b_abund * 0.3
        ]
    ...
    if lines_available[index_line] == "yes":
        O1c_abund_fake = O1.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['O1_6364A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(6364)',
            Hbeta=100.0)
        O1c_abund = O1.getIonAbundance(int_ratio=obs.getIntens(0)['O1_6364A'],
                                       tem=Teinput,
                                       den=Neinput,
                                       to_eval='L(6364)',
                                       Hbeta=100.0)
        ...
        O1c_abund_fake = [
            i2 for i2 in O1c_abund_fake
            if i2 < O1c_abund * 3 and i2 > O1c_abund * 0.3
        ]
    ...
    if (index1 == 1 and index2 == 0 and index3 == 0):
        ...
        O1_abund_fake = np.sqrt(O1a_abund_fake**2 + O1b_abund_fake**2 +
                                O1c_abund_fake**2) / 3.
        O1_abund_fake2 = (O1a_abund_fake1 + O1b_abund_fake2 +
                          O1c_abund_fake3) / 3.
    ...
    if np.isnan(O1a_abund_fake) == True:
        O1a_abund_fake = 0
    ion_abun_error.OIa[i] = O1a_abund_fake
    if np.isnan(O1b_abund_fake) == True:
        O1b_abund_fake = 0
    ion_abun_error.OIb[i] = O1b_abund_fake
    if np.isnan(O1c_abund_fake) == True:
        O1c_abund_fake = 0
    ion_abun_error.OIc[i] = O1c_abund_fake

    print('Oo(5577)/H+ = {:5.2e}'.format(ion_abun.OIa[i]),
          'Oo(6300)/H+ = {:5.2e}'.format(ion_abun.OIb[i]),
          'Oo(6363)/H+ = {:5.2e}'.format(ion_abun.OIc[i]),
          file=file10)
    print('Oo(5577)/H+ = {:5.2e}'.format(ion_abun_error.OIa[i]),
          'Oo(6300)/H+ = {:5.2e}'.format(ion_abun_error.OIb[i]),
          'Oo(6363)/H+ = {:5.2e}'.format(ion_abun_error.OIc[i]),
          file=file10)
    #############################################################
    ...
    if lines_available[index_line] == "yes":
        O2a_abund_fake = O2.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['O2_3726A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(3726)',
            Hbeta=100.0)
        O2a_abund = O2.getIonAbundance(int_ratio=obs.getIntens(0)['O2_3726A'],
                                       tem=Teinput,
                                       den=Neinput,
                                       to_eval='L(3726)',
                                       Hbeta=100.0)
        ...
        O2a_abund_fake = [
            i2 for i2 in O2a_abund_fake
            if i2 < O2a_abund * 3 and i2 > O2a_abund * 0.3
        ]
    ...
    if lines_available[index_line] == "yes":
        O2b_abund_fake = O2.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['O2_3729A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(3729)',
            Hbeta=100.0)
        O2b_abund = O2.getIonAbundance(int_ratio=obs.getIntens(0)['O2_3729A'],
                                       tem=Teinput,
                                       den=Neinput,
                                       to_eval='L(3729)',
                                       Hbeta=100.0)
        ...
        O2b_abund_fake = [
            i2 for i2 in O2b_abund_fake
            if i2 < O2b_abund * 3 and i2 > O2b_abund * 0.3
        ]
    ...
    if lines_available[index_line] == "yes":
        O2c_abund_fake = O2.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['O2_7319A+'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='I(4,2)+I(5,2)',
            Hbeta=100.0)
        O2c_abund = O2.getIonAbundance(int_ratio=obs.getIntens(0)['O2_7319A+'],
                                       tem=Teinput,
                                       den=Neinput,
                                       to_eval='I(4,2)+I(5,2)',
                                       Hbeta=100.0)
        ...
        O2c_abund_fake = [
            i2 for i2 in O2c_abund_fake
            if i2 < O2c_abund * 3 and i2 > O2c_abund * 0.3
        ]
    ...
    if lines_available[index_line] == "yes":
        O2d_abund_fake = O2.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['O2_7330A+'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='I(4,3)+I(5,3)',
            Hbeta=100.0)
        O2d_abund = O2.getIonAbundance(int_ratio=obs.getIntens(0)['O2_7330A+'],
                                       tem=Teinput,
                                       den=Neinput,
                                       to_eval='I(4,3)+I(5,3)',
                                       Hbeta=100.0)
        ...
        O2d_abund_fake = [
            i2 for i2 in O2d_abund_fake
            if i2 < O2d_abund * 3 and i2 > O2d_abund * 0.3
        ]
    ...
    if (index1 == 1 and index2 == 0 and index3 == 0 and index4 == 0):
        ...
        O2_abund_fake = np.sqrt(O2a_abund_fake**2 + O2b_abund_fake**2 +
                                O2c_abund_fake**2 + O2d_abund_fake**2) / 4.
        O2_abund_fake2 = (O2a_abund_fake1 + O2b_abund_fake2 + O2c_abund_fake3 +
                          O2d_abund_fake4) / 4.
    ...
    if np.isnan(O2a_abund_fake) == True:
        O2a_abund_fake = 0
    ion_abun_error.OIIa[i] = O2a_abund_fake
    if np.isnan(O2b_abund_fake) == True:
        O2b_abund_fake = 0
    ion_abun_error.OIIb[i] = O2b_abund_fake
    if np.isnan(O2c_abund_fake) == True:
        O2c_abund_fake = 0
    ion_abun_error.OIIc[i] = O2c_abund_fake
    if np.isnan(O2d_abund_fake) == True:
        O2d_abund_fake = 0
    ion_abun_error.OIId[i] = O2d_abund_fake

    print('O+(3727)/H+ = {:5.2e}'.format(ion_abun.OIIa[i]),
          'O+(3729)/H+ = {:5.2e}'.format(ion_abun.OIIb[i]),
          'O+(7320)/H+ = {:5.2e}'.format(ion_abun.OIIc[i]),
          'O+(7330)/H+ = {:5.2e}'.format(ion_abun.OIId[i]),
          file=file10)
    print('O+(3727)/H+ = {:5.2e}'.format(ion_abun_error.OIIa[i]),
          'O+(3729)/H+ = {:5.2e}'.format(ion_abun_error.OIIb[i]),
          'O+(7320)/H+ = {:5.2e}'.format(ion_abun_error.OIIc[i]),
          'O+(7330)/H+ = {:5.2e}'.format(ion_abun_error.OIId[i]),
          file=file10)
    ...
    if lines_available[index_line] == "yes":
        O3a_abund_fake = O3.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['O3_4363A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(4363)',
            Hbeta=100.0)
        O3a_abund = O3.getIonAbundance(int_ratio=obs.getIntens(0)['O3_4363A'],
                                       tem=Teinput,
                                       den=Neinput,
                                       to_eval='L(4363)',
                                       Hbeta=100.0)
        ...
        O3a_abund_fake = [
            i2 for i2 in O3a_abund_fake
            if i2 < O3a_abund * 3 and i2 > O3a_abund * 0.3
        ]
    ...
    if lines_available[index_line] == "yes":
        O3b_abund_fake = O3.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['O3_4959A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(4959)',
            Hbeta=100.0)
        O3b_abund = O3.getIonAbundance(int_ratio=obs.getIntens(0)['O3_4959A'],
                                       tem=Teinput,
                                       den=Neinput,
                                       to_eval='L(4959)',
                                       Hbeta=100.0)
        ...
        O3b_abund_fake = [
            i2 for i2 in O3b_abund_fake
            if i2 < O3b_abund * 3 and i2 > O3b_abund * 0.3
        ]
    ...
    if lines_available[index_line] == "yes":
        O3c_abund_fake = O3.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['O3_5007A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(5007)',
            Hbeta=100.0)
        O3c_abund = O3.getIonAbundance(int_ratio=obs.getIntens(0)['O3_5007A'],
                                       tem=Teinput,
                                       den=Neinput,
                                       to_eval='L(5007)',
                                       Hbeta=100.0)
        ...
        O3c_abund_fake = [
            i2 for i2 in O3c_abund_fake
            if i2 < O3c_abund * 3 and i2 > O3c_abund * 0.3
        ]
    ...
    if (index1 == 0 and index2 == 0 and index3 == 1):
        ...
        O3_abund_fake = np.sqrt(O3a_abund_fake**2 + O3b_abund_fake**2 +
                                O3c_abund_fake**2) / 3.
        O3_abund_fake2 = (O3a_abund_fake1 + O3b_abund_fake2 +
                          O3c_abund_fake3) / 3.
    ...
    if np.isnan(O3a_abund_fake) == True:
        O3a_abund_fake = 0
    ion_abun_error.OIIIa[i] = O3a_abund_fake
    if np.isnan(O3b_abund_fake) == True:
        O3b_abund_fake = 0
    ion_abun_error.OIIIb[i] = O3b_abund_fake
    if np.isnan(O3c_abund_fake) == True:
        O3c_abund_fake = 0
    ion_abun_error.OIIIc[i] = O3c_abund_fake

    print('O+2(4363)/H+ = {:5.2e}'.format(ion_abun.OIIIa[i]),
          'O+2(4959)/H+ = {:5.2e}'.format(ion_abun.OIIIb[i]),
          'O+2(5007)/H+ = {:5.2e}'.format(ion_abun.OIIIc[i]),
          file=file10)
    print('O+2(4363)/H+ = {:5.2e}'.format(ion_abun_error.OIIIa[i]),
          'O+2(4959)/H+ = {:5.2e}'.format(ion_abun_error.OIIIb[i]),
          'O+2(5007)/H+ = {:5.2e}'.format(ion_abun_error.OIIIc[i]),
          file=file10)
    #############################################################
...
#        N1a_abund=[0.0,0.0]
    ...
    index_line = line_names.index("N1_5199s")
    if lines_available[index_line] == "yes":
        N1b_abund_fake = N1.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['N1_5200A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(5200)',
            Hbeta=100.0)
        N1b_abund = N1.getIonAbundance(int_ratio=obs.getIntens(0)['N1_5200A'],
                                       tem=Teinput,
                                       den=Neinput,
                                       to_eval='L(5200)',
                                       Hbeta=100.0)
        ...
        N1b_abund_fake = [
            i2 for i2 in N1b_abund_fake
            if i2 < N1b_abund * 3 and i2 > N1b_abund * 0.3
        ]
    ...
    if np.isnan(N1b_abund_fake) == True:
        N1b_abund_fake = 0
    ...
    if lines_available[index_line] == "yes":
        N2a_abund_fake = N2.getIonAbundance(
            int_ratio=obs_fake.getIntens()['N2_5755A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(5755)',
            Hbeta=100.0)
        N2a_abund = N2.getIonAbundance(int_ratio=obs.getIntens()['N2_5755A'],
                                       tem=Teinput,
                                       den=Neinput,
                                       to_eval='L(5755)',
                                       Hbeta=100.0)
        ...
        N2a_abund_fake = [
            i2 for i2 in N2a_abund_fake
            if i2 < N2a_abund * 3 and i2 > N2a_abund * 0.3
        ]
    ...
    if lines_available[index_line] == "yes":
        N2b_abund_fake = N2.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['N2_6548A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(6548)',
            Hbeta=100.0)
        N2b_abund = N2.getIonAbundance(int_ratio=obs.getIntens(0)['N2_6548A'],
                                       tem=Teinput,
                                       den=Neinput,
                                       to_eval='L(6548)',
                                       Hbeta=100.0)
        ...
        N2b_abund_fake = [
            i2 for i2 in N2b_abund_fake
            if i2 < N2b_abund * 3 and i2 > N2b_abund * 0.3
        ]
    ...
    if lines_available[index_line] == "yes":
        N2c_abund_fake = N2.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['N2_6584A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(6584)',
            Hbeta=100.0)
        N2c_abund = N2.getIonAbundance(int_ratio=obs.getIntens(0)['N2_6584A'],
                                       tem=Teinput,
                                       den=Neinput,
                                       to_eval='L(6584)',
                                       Hbeta=100.0)
        ...
        N2c_abund_fake = [
            i2 for i2 in N2c_abund_fake
            if i2 < N2c_abund * 3 and i2 > N2c_abund * 0.3
        ]
    ...
    if (index1 == 0 and index2 == 0 and index3 == 1):
        ...
        N2_abund_fake = np.sqrt(N2a_abund_fake**2 + N2b_abund_fake**2 +
                                N2c_abund_fake**2) / 3.
        N2_abund_fake2 = (N2a_abund_fake1 + N2b_abund_fake2 +
                          N2c_abund_fake3) / 3.
    ...
    if np.isnan(N2a_abund_fake) == True:
        N2a_abund_fake = 0
    ion_abun_error.NIIa[i] = N2a_abund_fake
    if np.isnan(N2b_abund_fake) == True:
        N2b_abund_fake = 0
    ion_abun_error.NIIb[i] = N2b_abund_fake
    if np.isnan(N2c_abund_fake) == True:
        N2c_abund_fake = 0
    ion_abun_error.NIIc[i] = N2c_abund_fake

    print('N+(5755)/H+ = {:5.2e}'.format(ion_abun.NIIa[i]),
          'N+(6548)/H+ = {:5.2e}'.format(ion_abun.NIIb[i]),
          'N+(6584)/H+ = {:5.2e}'.format(ion_abun.NIIc[i]),
          file=file10)
    print('N+(5755)/H+ = {:5.2e}'.format(ion_abun_error.NIIa[i]),
          'N+(6548)/H+ = {:5.2e}'.format(ion_abun_error.NIIb[i]),
          'N+(6584)/H+ = {:5.2e}'.format(ion_abun_error.NIIc[i]),
          file=file10)
    #############################################################
    ...
    if lines_available[index_line] == "yes":
        S2a_abund_fake = S2.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['S2_6716A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(6717)',
            Hbeta=100.0)
        S2a_abund = S2.getIonAbundance(int_ratio=obs.getIntens(0)['S2_6716A'],
                                       tem=Teinput,
                                       den=Neinput,
                                       to_eval='L(6717)',
                                       Hbeta=100.0)
        ...
        S2a_abund_fake = [
            i2 for i2 in S2a_abund_fake
            if i2 < S2a_abund * 3 and i2 > S2a_abund * 0.3
        ]
    ...
    if lines_available[index_line] == "yes":
        S2b_abund_fake = S2.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['S2_6731A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(6731)',
            Hbeta=100.0)
        S2b_abund = S2.getIonAbundance(int_ratio=obs.getIntens(0)['S2_6731A'],
                                       tem=Teinput,
                                       den=Neinput,
                                       to_eval='L(6731)',
                                       Hbeta=100.0)
        ...
        S2b_abund_fake = [
            i2 for i2 in S2b_abund_fake
            if i2 < S2b_abund * 3 and i2 > S2b_abund * 0.3
        ]
    ...
    if np.isnan(S2a_abund_fake) == True:
        S2a_abund_fake = 0
    ion_abun_error.SIIa[i] = S2a_abund_fake
    if np.isnan(S2b_abund_fake) == True:
        S2b_abund_fake = 0
    ion_abun_error.SIIb[i] = S2b_abund_fake

    print('S+(6717)/H+ = {:5.2e}'.format(ion_abun.SIIa[i]),
          'S+(6731)/H+ = {:5.2e}'.format(ion_abun.SIIb[i]),
          file=file10)
    print('S+(6717)/H+ = {:5.2e}'.format(ion_abun_error.SIIa[i]),
          'S+(6731)/H+ = {:5.2e}'.format(ion_abun_error.SIIb[i]),
          file=file10)
    #############################################################
    ...
    if lines_available[index_line] == "yes":
        S3a_abund_fake = S3.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['S3_6312A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(6312)',
            Hbeta=100.0)
        S3a_abund = S3.getIonAbundance(int_ratio=obs.getIntens(0)['S3_6312A'],
                                       tem=Teinput,
                                       den=Neinput,
                                       to_eval='L(6312)',
                                       Hbeta=100.0)
        ...
        S3a_abund_fake = [
            i2 for i2 in S3a_abund_fake
            if i2 < S3a_abund * 3 and i2 > S3a_abund * 0.3
        ]
    ...
    if lines_available[index_line] == "yes":
        S3b_abund_fake = S3.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['S3_9069A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(9069)',
            Hbeta=100.0)
        S3b_abund = S3.getIonAbundance(int_ratio=obs.getIntens(0)['S3_9069A'],
                                       tem=Teinput,
                                       den=Neinput,
                                       to_eval='L(9069)',
                                       Hbeta=100.0)
        ...
        S3b_abund_fake = [
            i2 for i2 in S3b_abund_fake
            if i2 < S3b_abund * 3 and i2 > S3b_abund * 0.3
        ]
...
#S3b_abund_fake=np.std(np.nan_to_num(S3b_abund_fake), ddof=1)
    ...
    if np.isnan(S3a_abund_fake) == True:
        S3a_abund_fake = 0
    ion_abun_error.SIIIa[i] = S3a_abund_fake
    if np.isnan(S3b_abund_fake) == True:
        S3b_abund_fake = 0
    ...
    print('S+2(6312)/H+ = {:5.2e}'.format(ion_abun.SIIIa[i]),
          'S+2(9068)/H+ = {:5.2e}'.format(ion_abun.SIIIb[i]),
          file=file10)
    print('S+2(6312)/H+ = {:5.2e}'.format(ion_abun_error.SIIIa[i]),
          'S+2(9068)/H+ = {:5.2e}'.format(ion_abun_error.SIIIb[i]),
          file=file10)
    ...
    if lines_available[index_line] == "yes":
        Ne3a_abund_fake = Ne3.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['Ne3_3869A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(3869)',
            Hbeta=100.0)
        Ne3a_abund = Ne3.getIonAbundance(
            int_ratio=obs.getIntens(0)['Ne3_3869A'],
            tem=Teinput,
            den=Neinput,
            to_eval='L(3869)',
            Hbeta=100.0)
        ...
        Ne3a_abund_fake = [
            i2 for i2 in Ne3a_abund_fake
            if i2 < Ne3a_abund * 3 and i2 > Ne3a_abund * 0.3
        ]
    ...
    if lines_available[index_line] == "yes":
        Ne3b_abund_fake = Ne3.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['Ne3_3968A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(3968)',
            Hbeta=100.0)
        Ne3b_abund = Ne3.getIonAbundance(
            int_ratio=obs.getIntens(0)['Ne3_3968A'],
            tem=Teinput,
            den=Neinput,
            to_eval='L(3968)',
            Hbeta=100.0)
        ...
        Ne3b_abund_fake = [
            i2 for i2 in Ne3b_abund_fake
            if i2 < Ne3b_abund * 3 and i2 > Ne3b_abund * 0.3
        ]
    ...
    if np.isnan(Ne3a_abund_fake) == True:
        Ne3a_abund_fake = 0
    ion_abun_error.NeIIIa[i] = Ne3a_abund_fake
    if np.isnan(Ne3b_abund_fake) == True:
        Ne3b_abund_fake = 0
    ion_abun_error.NeIIIb[i] = Ne3b_abund_fake

    print('Ne+2(3869)/H+ = {:5.2e}'.format(ion_abun.NeIIIa[i]),
          'Ne+2(3968)/H+ = {:5.2e}'.format(ion_abun.NeIIIb[i]),
          file=file10)
    print('Ne+2(3869)/H+ = {:5.2e}'.format(ion_abun_error.NeIIIa[i]),
          'Ne+2(3968)/H+ = {:5.2e}'.format(ion_abun_error.NeIIIb[i]),
          file=file10)
    ...
    if lines_available[index_line] == "yes":
        Ar3a_abund_fake = Ar3.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['Ar3_7136A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(7136)',
            Hbeta=100.0)
        Ar3a_abund = Ar3.getIonAbundance(
            int_ratio=obs.getIntens(0)['Ar3_7136A'],
            tem=Teinput,
            den=Neinput,
            to_eval='L(7136)',
            Hbeta=100.0)
        ...
        Ar3a_abund_fake = [
            i2 for i2 in Ar3a_abund_fake
            if i2 < Ar3a_abund * 3 and i2 > Ar3a_abund * 0.3
        ]
    ...
    if lines_available[index_line] == "yes":
        Ar4a_abund_fake = Ar4.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['Ar4_4711A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(4711)',
            Hbeta=100.0)
        Ar4a_abund = Ar4.getIonAbundance(
            int_ratio=obs.getIntens(0)['Ar4_4711A'],
            tem=Teinput,
            den=Neinput,
            to_eval='L(4711)',
            Hbeta=100.0)
        ...
        Ar4a_abund_fake = [
            i2 for i2 in Ar4a_abund_fake
            if i2 < Ar4a_abund * 3 and i2 > Ar4a_abund * 0.3
        ]
    ...
    if lines_available[index_line] == "yes":
        Ar4b_abund_fake = Ar4.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['Ar4_4740A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(4740)',
            Hbeta=100.0)
        Ar4b_abund = Ar4.getIonAbundance(
            int_ratio=obs.getIntens(0)['Ar4_4740A'],
            tem=Teinput,
            den=Neinput,
            to_eval='L(4740)',
            Hbeta=100.0)
        ...
        Ar4b_abund_fake = [
            i2 for i2 in Ar4b_abund_fake
            if i2 < Ar4b_abund * 3 and i2 > Ar4b_abund * 0.3
        ]
    ...
    if np.isnan(Ar4a_abund_fake) == True:
        Ar4a_abund_fake = 0
    ion_abun_error.ArIVa[i] = Ar4a_abund_fake
    if np.isnan(Ar4b_abund_fake) == True:
        Ar4b_abund_fake = 0
    ion_abun_error.ArIVb[i] = Ar4b_abund_fake

    print('Ar+3(4712)/H+ = {:5.2e}'.format(ion_abun.ArIVa[i]),
          'Ar+3(4740)/H+ = {:5.2e}'.format(ion_abun.ArIVb[i]),
          file=file10)
    print('Ar+3(4712)/H+ = {:5.2e}'.format(ion_abun_error.ArIVa[i]),
          'Ar+3(4740)/H+ = {:5.2e}'.format(ion_abun_error.ArIVb[i]),
          file=file10)
    #############################################################
    ...
    if lines_available[index_line] == "yes":
        Cl3a_abund_fake = Cl3.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['Cl3_5518A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(5518)',
            Hbeta=100.0)
        Cl3a_abund = Cl3.getIonAbundance(
            int_ratio=obs.getIntens(0)['Cl3_5518A'],
            tem=Teinput,
            den=Neinput,
            to_eval='L(5518)',
            Hbeta=100.0)
        ...
        Cl3a_abund_fake = [
            i2 for i2 in Cl3a_abund_fake
            if i2 < Cl3a_abund * 3 and i2 > Cl3a_abund * 0.3
        ]
    ...
    if lines_available[index_line] == "yes":
        Cl3b_abund_fake = Cl3.getIonAbundance(
            int_ratio=obs_fake.getIntens(0)['Cl3_5538A'],
            tem=Teinput_fake,
            den=Neinput_fake,
            to_eval='L(5538)',
            Hbeta=100.0)
        Cl3b_abund = Cl3.getIonAbundance(
            int_ratio=obs.getIntens(0)['Cl3_5538A'],
            tem=Teinput,
            den=Neinput,
            to_eval='L(5538)',
            Hbeta=100.0)
        ...
        Cl3b_abund_fake = [
            i2 for i2 in Cl3b_abund_fake
            if i2 < Cl3b_abund * 3 and i2 > Cl3b_abund * 0.3
        ]
    ...
    if np.isnan(Cl3a_abund_fake) == True:
        Cl3a_abund_fake = 0
    ion_abun_error.ClIIIa[i] = Cl3a_abund_fake
    if np.isnan(Cl3b_abund_fake) == True:
        Cl3b_abund_fake = 0
    ion_abun_error.ClIIIb[i] = Cl3b_abund_fake

    print('Cl+2(5517)/H+ = {:5.2e}'.format(ion_abun.ClIIIa[i]),
          'Cl+2(5538)/H+ = {:5.2e}'.format(ion_abun.ClIIIb[i]),
          file=file10)
    print('Cl+2(5517)/H+ = {:5.2e}'.format(ion_abun_error.ClIIIa[i]),
          'Cl+2(5538)/H+ = {:5.2e}'.format(ion_abun_error.ClIIIb[i]),
          file=file10)
    #############################################################
