    ...
    for element in dct['data_list']['element_list']:
        for spec in element['spectrums']:
            for atomic in spec['atomic_numbers']:
                sfn = element['atom'] + spec['spectrum'] + '_' + \
                    str(atomic['atomic_number']) + did + \
                    '.' + dct['data_list']['suffix']
                efn = element['atom'] + spec['spectrum'] + '_' + \
                    str(atomic['atomic_number']) + eid + \
                    '.' + dct['data_list']['suffix']
                fns.append({'element': element['atom'], 'spectrum': spec['spectrum'].lower(), 'atomic': atomic['atomic_number'], 'ref_tene': atomic['ref_TeNe'], 'fns': os.path.join(
                    dct['data_list']['prefix'], sfn), 'fne': os.path.join(dct['data_list']['prefix'], efn)})
    return fns
...
def elementInputDict(fitsd: list, reference_element: dict, logger=None):
    ...
    if logger is not None:
        logger.error("Failed matching element {:}/{:}/{:} to available input file".format(
            reference_element['element'], reference_element['spectrum'], reference_element['atomic']))
    raise RuntimeError("[ERROR] Failed matching element {:}/{:}/{:} to available input file".format(
        reference_element['element'], reference_element['spectrum'], reference_element['atomic']))
...
# return a list of dictionaries, one for each slit
    ...
    for slit in d['slits']:
        ...
        slits.append({'PA': ps[0], 'w': ps[1],
                     'h': ps[2], 'x': ps[3], 'y': ps[4]})
    return slits
...
# replace roman spectrum with int and see if file exists
                if bn.find(sstr) >= 0:
                    ...
                    if os.path.isfile(guess):
                        ...
                        if logger:
                            logger.debug("Fits filename {:} is missing; using {:}".format(
...
def configRefTenNe2PyNebPair(ref_tene):
    ...
    """
    match = re.match(
        r'\[([A-Za-z0-9]*)\]\s*([0-9a-zA-Z\+]*[/0-9a-zA-Z\+]*)\s*\[([A-Za-z0-9]*)\]\s*([0-9a-zA-Z\+]*[/0-9a-zA-Z\+]*)', ref_tene)
    return "[{:}] {:}".format(match.group(1), match.group(2)), "[{:}] {:}".format(match.group(3), match.group(4))
